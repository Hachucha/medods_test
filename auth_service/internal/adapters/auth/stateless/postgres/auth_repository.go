...
func (r *PostgresAuthRepository) GetSession(ctx context.Context, userID stateless.UserID, refreshHash string) (stateless.SessionData, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT user_id, token_pair_id, refresh_hash, user_agent, ip FROM `+r.conf.Prefix+`sls_auth_sessions WHERE user_id = $1 AND refresh_hash = $2`, userID, refreshHash)
